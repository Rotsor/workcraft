...
class PluginManifestTest extends Spec {
  ...
  describe("PluginManifest") {
    ...
    it("should report an empty manifest as error") {
      PluginManifest.read(guid1, getClass.getResource("/emptyManifest").getPath()) match {
        case Right(_) => fail("expected error value")
        case Left(error) => error match {
    ...
    it("should report a version mismatch") {
      PluginManifest.read(guid1, getClass().getResource("/wrongVersion").getPath()) match {
        case Right(_) => fail("expected error value")
        case Left(error) => error match {
    ...
    it("should correctly read a well-formed manifest") {
      PluginManifest.read(guid1, getClass().getResource("/goodManifest").getPath()) match {
