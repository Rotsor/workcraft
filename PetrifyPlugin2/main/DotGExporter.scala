...
object DotGExporter extends Exporter {
  ...
  def export(model: ModelServiceProvider): Either[ServiceNotAvailableException, ExportJob] = model.implementation(PetriNetService) match {
    case Some(impl) => Right(new PnToDotGExportJob(impl, None))
    case None => Left(new ServiceNotAvailableException(PetriNetService))
  }
}

class PnToDotGExportJob(snapshot: IO[PetriNet], capacity : Option[Int]) extends ExportJob {
  val complete = false

  def job(file: File) = snapshot >>= (net => IO {
    try {
      val writer: PrintWriter = new PrintWriter(new BufferedOutputStream(new FileOutputStream(file)))
      ...
      try {
        ...
        capacity match {
          case None => {}
          case Some(n) => writer.println (".capacity " + net.places.map(p => net.labelling(p)+"="+n.toString).mkString(" "))
        }
